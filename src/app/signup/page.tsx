...
const SignupPage = () => {
  ...
  const onGoogle = async () => {
    ...
    try {
      if (usingEmulator) {
        setError("OAuth non disponibile in modalità emulatore. Usa email/password per il test locale.");
        return;
      }
      await signInGoogle();
      router.push("/");
  ...
  const onApple = async () => {
    ...
    try {
      ...
      setError(err?.message || "Accesso con Apple non riuscito");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="container mx-auto px-4 py-10">
        <div className="mx-auto w-full max-w-md">
          <div className="rounded-lg bg-white dark:bg-dark shadow p-6">
            <h1 className="text-2xl font-bold text-center mb-2">Crea un account</h1>
            <p className="text-center text-sm text-gray-600 dark:text-gray-300 mb-6">
              Registrati per salvare corsi e inviare richieste in un attimo.
            </p>

            <div className="space-y-3 mb-4">
                <button onClick={onGoogle} disabled={loading} className="btn btn-outline w-full">
                <span className="mr-2">
                ...
                <button
                  onClick={onApple}
                  disabled={loading}
                  className="btn w-full bg-black hover:bg-gray-800 text-white border-black hover:border-gray-800 dark:bg-white dark:text-black dark:hover:bg-gray-100 dark:border-white dark:hover:border-gray-100"
                >
                <span className="mr-2">{/* Apple icon */}
            ...
            <p className="text-center text-sm mt-4">
              Hai già un account? <Link className="link link-primary" href="/signin">Accedi</Link>
            </p>
          </div>
        </div>
      </div>
